Shader "Unlit URP Shader"
{
    Properties
    {
        ...
        _starColorIntensity("星星颜色强度", Range(0, 50)) = 0.8466
...
/*
                 float2 _irradianceMap_R_uv;
                    _irradianceMap_R_uv.x = abs(_angle_up_to_down_1_n1) / max(_IrradianceMapR_maxAngleRange,0.001f);
                 ...
                 float _VDotSunDampingA = max(0, lerp( 1, SunDirection, _mainColorSunGatherFactor ));
...
*/
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                // Light mainLight = GetMainLight();
                // float sunDist = distance(i.UV.xyz, mainLight.direction);
                ...
                // float moonArea444 = smoothstep(0.5,1,moonArea);
               // float moonArea444 = clamp(moonArea,0,1);
              ...
              //   float  _WorldPosDotUpstep1 = smoothstep(-1,1,_WorldPosDotUp);
               ...
               _WorldPosDotUpstep1 = smoothstep(0.4,1,_WorldPosDotUpstep1 );
                ...
                //float _WorldPosDotUpstep3 = smoothstep(0,1, _WorldPosDotUp );
              ...
              // float  _WorldPosDotSun = dot(normalize(mainLight.direction.xyz), i.Varying_WorldPosAndAngle.xyz);
          
                 
              //  float3 _viewDirNormalize = normalize(i.Varying_WorldPosAndAngle.xyz);
             
                 // float  _WorldPosDotUp_Multi999 = abs(_WorldPosDotUp) * _sun_disk_power_999;
               
           

              // float  _WorldPosDotSunRemap01Clamp = clamp(_WorldPosDotSun * 0.5 + 0.5, 0, 1);//太阳方向
              // float  _WorldPosDotSunRemap = clamp(sunArea * 0.5 + 0.5, 0, 1);//太阳方向
              ...
              //  float3 TT = pow(sunArea3 , _WorldPosDotUp_Multi999 * float3(1, 0.1, 0.01));
                
                float3 _sun_disk_sunArea = sunArea * _sun_color_intensity * _sun_color ;
                _sun_disk = _sun_disk + _sun_disk_sunArea * 3;
                //float3(1, 0.1, 0.01)float3(1, 0.12, 0.03)1, 0.3, 0.05,_WorldPosDotUp_Multi999 * float3(1, 0.1, 0.01)
                float _LDotDirClampn11_smooth = smoothstep(0, 1, sunArea3);
              //  i.Varying_IrradianceColor.xyz = min(i.Varying_IrradianceColor.xyz,_sun_disk_sunArea*3);
              ...
              float3 galaxyColor =clamp((galaxyTex.xyz*galaxyTex.w*_WorldPosDotUp *0.2),0,1);
            // return float4(galaxyTex.xyz*galaxyTex.w*_WorldPosDotUp *0.3,1);
           // return float4(galaxyTex.w,galaxyTex.w,galaxyTex.w ,1);
